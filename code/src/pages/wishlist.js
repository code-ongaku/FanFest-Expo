...
export default function Wishlist() {
  ...
  const filteredBookmarks = Bookmarks.filter((bookmark, index) => {
    ...
    const dayFilter =
      selectedDay === "all" ||
      selectedDay === "All" ||
      bookmark.day === selectedDay;
    const notTrashed = !isTrashed[index];
    return categoryFilter && dayFilter && notTrashed;
  });

  // A simple helper function to map category to a background color on the filter buttons
  ...
  return (
    <div className="bg-[#F8F1FF] text-[#7E52A0] min-h-screen w-screen font-roboto-slab flex flex-col items-center">
      ...
      <div className="mt-3 w-[90%] max-w-md">
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${
                    isSelected
                      ? "ring-2 ring-black" // highlight selected
                      : ""
                  }
        ...
        {/* Day Filters (Add py-2 if you want more vertical space) */}
        <div className="mt-4 flex justify-around">
          {["All", "Friday", "Saturday", "Sunday"].map((day) => (
            <button
              ...
              onClick={() =>
                setSelectedDay(day === selectedDay ? "all" : day)
              }
            >
              {day}
            </button>
